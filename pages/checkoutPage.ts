...
export class CheckoutPage {
  ...
  private async verifyAddress(selectorPrefix: string, firstName: string, lastName: string, user: User) {

    // Name
    const nameLocator = this.page.locator(`${selectorPrefix} .address_firstname.address_lastname`);
    await expect(nameLocator).toContainText(`${firstName} ${lastName}`);

    // Company
    const addressLines = this.page.locator(`${selectorPrefix} .address_address1.address_address2`);
    ...
    // City, State, Zip
    const cityStateZipLocator = this.page.locator(`${selectorPrefix} .address_city.address_state_name.address_postcode`);
    ...
    // Country
    const countryLocator = this.page.locator(`${selectorPrefix} .address_country_name`);
  ...
  async verifyDeliveryAddress(firstName: string, lastName: string) {
    await this.verifyAddress('#address_delivery', firstName ?? '', lastName ?? '', user);
  }

  async verifyBillingAddress(firstName: string, lastName: string) {
    await this.verifyAddress('#address_invoice',  firstName ?? '', lastName ?? '', user);
  }
  ...
  async enterDescriptionAndPlaceOrder() {
    await this.page.locator('textarea[name="message"]').fill('Please deliver between 9 AM to 5 PM');
